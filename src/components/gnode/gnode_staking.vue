<template>
  <div class="service-node-staking">
    <div class="q-px-md q-pt-md">
      <p class="tab-desc">
        ...
        <span
          ...
          @click="gyuanxWebsite"
          >Gyuanx {{ $t("strings.website") }}.</span
        >
      </p>
      <GyuanxField
        :label="$t('fieldLabels.serviceNodeKey')"
        :error="$v.gnode.key.$error"
      >
        <q-input
          v-model.trim="gnode.key"
          :dark="theme == 'dark'"
          :placeholder="$t('placeholders.hexCharacters', { count: 64 })"
          borderless
          dense
          @blur="$v.gnode.key.$touch"
        />
      </GyuanxField>
      <GyuanxField
        :label="$t('fieldLabels.amount')"
        class="q-mt-md"
        :error="$v.gnode.amount.$error"
      >
        <q-input
          v-model.trim="gnode.amount"
          :dark="theme == 'dark'"
          type="number"
          min="0"
        ...
        <q-btn
          ...
          @click="gnode.amount = minStake(gnode.key)"
        />
        <q-btn
          color="primary"
          :text-color="theme == 'dark' ? 'white' : 'dark'"
          :label="$t('buttons.max')"
          :disable="!areButtonsEnabled()"
          @click="gnode.amount = maxStake(gnode.key)"
        />
      </GyuanxField>
    ...
    <ServiceNodeContribute
      :awaiting-service-nodes="awaiting_gnodes"
      class="contribute"
...
import { gnode_key, greater_than_zero } from "src/validators/common";
import GyuanxField from "components/gyuanx_field";
import WalletPassword from "src/mixins/wallet_password";
import ConfirmDialogMixin from "src/mixins/confirm_dialog_mixin";
import ServiceNodeContribute from "./gnode_contribute";
import ServiceNodeMixin from "src/mixins/gnode_mixin";
...
export default {
  ...
  computed: mapState({
    ...
    stake_status: state => state.gateway.gnode_status.stake,
    ...
    awaiting_gnodes(state) {
      const nodes = state.gateway.daemon.gnodes.nodes;
  ...
  methods: {
    gyuanxWebsite() {
      const url = "https://gyuan.online/service-nodes/";
    ...
    fillStakingFields(key, minContribution) {
      this.gnode.key = key;
      this.gnode.amount = minContribution;
    },
    minStake() {
    ...
    maxStake() {
      const node = this.getNodeWithPubKey();
      return this.openForContriubtionGyuanx(node);
    },
    getFeeDecimal(node) {
    ...
    getNodeWithPubKey() {
      const key = this.gnode.key;
      const nodeOfKey = this.awaiting_gnodes.find(
        n => n.gnode_pubkey === key
      );
      if (!nodeOfKey) {
        this.$q.notify({
          type: "negative",
    ...
    areButtonsEnabled() {
      ...
      const key = this.gnode.key;
      return !!this.awaiting_gnodes.find(
        n => n.gnode_pubkey === key
      );
    },
    async sweepAll() {
    ...
    async stake() {
      ...
      if (this.gnode.amount < 0) {
        ...
        return;
      } else if (this.$v.gnode.amount.$error) {
        this.$q.notify({
          type: "negative",
          timeout: 1000,
      ...
      passwordDialog
        .onOk(password => {
          ...
          const gnode = objectAssignDeep.noMutate(this.gnode, {
            password,
            destination: this.award_address
          });

          this.$gateway.send("wallet", "stake", gnode);
        })
        .onDismiss(() => {})
        .onCancel(() => {});
    }
